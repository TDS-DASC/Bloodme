describe('Load appointments index', () => {
    it('successfully logs in and load index', () => {
        ...
        cy.visit('http://localhost.local:5173/agents/36/edit');
        ...
        cy.get('input[name=name]').clear().type('Eugenio Test').should('have.value', 'Eugenio Test');
        cy.get('input[name=lastname]').clear().type('Cypress').should('have.value', 'Cypress');
        cy.get('input[name=alias]').clear().type('Cypress Testing Subject').should('have.value', 'Cypress Testing Subject');
        cy.get('input[type=date]').clear().type(formattedDate).should('have.value', formattedDate);
        ...
        cy.get('input[name=phone]').clear().type(phoneNumber).should('have.value', phoneNumber);
        const currentTime = new Date().getTime();
        const curp = `CURPF${currentTime}`.substring(0, 18);
        cy.get('input[name=curp]').clear();
